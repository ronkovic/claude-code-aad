...
pub fn execute(phase_str: &str) -> Result<()> {
    execute_with_paths(phase_str, ".aad/data/specs", ".aad/data/tasks")
}

/// Executes the gate command with custom repository paths (for testing).
fn execute_with_paths(phase_str: &str, spec_path: &str, task_path: &str) -> Result<()> {
    ...
    let spec_repo = SpecJsonRepo::new(spec_path);
    let task_repo = TaskJsonRepo::new(task_path);
...
mod tests {
    ...
    fn create_test_spec(temp_dir: &TempDir, spec_id_str: &str, description: &str) -> SpecId {
        let spec = Spec::new(spec_id_str.to_string(), description.to_string()).unwrap();
        // Use the actual spec.id generated by Spec::new(), not a manually created one
    ...
    fn create_test_task(temp_dir: &TempDir, spec_id: &SpecId, title: &str) {
        let task = Task::new(
            spec_id.clone(),
            title.to_string(),
        ...
        let tasks_dir = temp_dir.path().join(".aad/data/tasks");
        fs::create_dir_all(&tasks_dir).unwrap();

        // TaskJsonRepo stores tasks by task.id, not by spec_id subdirectory
    ...
    fn test_execute_spec_phase_passes() {
        ...
        let _spec_id = create_test_spec(&temp_dir, "SPEC-001", "Valid description for testing");

        let spec_path = temp_dir.path().join(".aad/data/specs").to_str().unwrap().to_string();
        let task_path = temp_dir.path().join(".aad/data/tasks").to_str().unwrap().to_string();

        let result = execute_with_paths("SPEC", &spec_path, &task_path);
    ...
    fn test_execute_spec_phase_fails_empty_description() {
        ...
        let _spec_id = create_test_spec(&temp_dir, "SPEC-001", "");
    ...
    fn test_execute_tasks_phase_passes() {
        ...
        let spec_id = create_test_spec(&temp_dir, "SPEC-001", "Valid spec");
        create_test_task(&temp_dir, &spec_id, "Task 1");
        create_test_task(&temp_dir, &spec_id, "Task 2");

        let spec_path = temp_dir.path().join(".aad/data/specs").to_str().unwrap().to_string();
        let task_path = temp_dir.path().join(".aad/data/tasks").to_str().unwrap().to_string();

        let result = execute_with_paths("TASKS", &spec_path, &task_path);
    ...
    fn test_execute_tasks_phase_fails_no_tasks() {
        ...
        let _spec_id = create_test_spec(&temp_dir, "SPEC-001", "Valid spec");
    ...
    fn test_execute_tdd_phase_passes() {
        ...
        let result = execute_with_paths("TDD", &spec_path, &task_path);
    ...
    fn test_execute_invalid_phase() {
        ...
        let result = execute_with_paths("INVALID", &spec_path, &task_path);
