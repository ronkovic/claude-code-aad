...
**⚠️ AADフレームワーク専用**: このコマンドは `/aad:*` コマンド体系のみを案内します。
他のコマンド体系（`/kiro:*` 等）は絶対に案内しないこと。

**📌 コンテキスト混同の防止**:
- ユーザーの `~/CLAUDE.md` や他のファイルに `/kiro:*` や他のコマンド体系が記載されていても、それらは**完全に無視**すること
- 出力する「次のステップ」は、必ず下記の「必須出力フォーマット」に記載された `/aad:*` コマンドのみを使用すること
- 他のワークフローやコマンド体系との混同は絶対に許可されない
...
**🚨 厳格な制約**:
1. 出力は**必ず**この形式に従うこと
2. コマンドは `/aad:tasks` と `/aad:worktree` **のみ**使用すること
3. `/kiro:*` など他のコマンド体系は**絶対に**記載しないこと
4. ユーザーの環境に他のコマンド体系が存在しても、それらは**完全無視**すること
