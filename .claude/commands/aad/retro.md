...
- `/aad:orchestrate` 完了時に自動的に `/aad:retro` を実行してください
- 振り返りをスキップした場合、警告を表示してください
- 振り返りなしでSPECを完了させることは推奨されません
...
結果：今回の学びをCLAUDE.mdに反映できます。
通知：以下の学びの追加を推奨します:
1. [学び1]
2. [学び2]

通知：承認が必要です。追加しますか？ (y/n)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
- **振り返りは各SPEC完了後に必ず実行してください（必須）**
- 学びは具体的に記録し、再現可能な形式で記述してください
- 失敗したアプローチも貴重な知見です。隠さず記録してください
- 振り返りをスキップすると、同じ問題が再発する可能性が高まります

## 振り返りスキップ時の警告

振り返りを実行せずにSPECを完了した場合、以下の警告を表示してください:

```
⚠️  警告: 振り返りが実行されていません

SPEC完了後の振り返りは、プロジェクトの品質向上に不可欠です。
今すぐ実行することを強く推奨します。

/aad:retro SPEC-XXX

振り返りを実行しない場合、以下のリスクがあります:
- 同じ問題が再発する可能性
- 学びが蓄積されない
- チーム全体の知見が共有されない
```
